...
export default class DocumentInput extends Component {
    ...
    onDrop(files) {
        ...
        for (var i = 0; i < files.length; i++) {
            console.log(files[i], 'what file',this.indexOfFile(files[i]), JSON.stringify(this.state.files).indexOf(JSON.stringify(files[i])), this.state.files)
            if (this.indexOfFile(files[i]) === - 1) {
                console.log(this.indexOfFile(files[i]), i,'here',this.state.files.concat(files))
        ...
        this.setState({ files: new_files }, () => {
            console.log(this.state.files, 'dealersOverallTotal1');
            this.props.updateCallback('files',this.state.files)
        });
        // console.log(this.state.files,new_files)
        // this.props.updateCallback('files',this.state.files)
    }
    onCancel(file) {
